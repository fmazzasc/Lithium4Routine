                    ...
                    bool debug = false, std::string path = "/data3/fmazzasc/sim/lit_new/", std::string outsuffix = "")
{
    ...
    std::string outFileName = "../../lithium_results/DauTreeMC" + outsuffix + ".root";
    ...
    std::sort(dirnums.begin(), dirnums.end());
    o2::base::GeometryManager::loadGeometry(path + "tf" + dirnums[0] + "/o2sim_geometry.root");
    ...
    for (unsigned int i = 0; i < dirnums.size(); i++)
    {
        ...
        std::string dir = path + "tf" + std::to_string(dirnum);
        std::string kine_file = "sgn_" + std::to_string(dirnum) + "_Kine.root";
        LOG(info) << "Processing " << dir;
        LOG(info) << "File # " << counter << " of " << dirnums.size();
